jQuery(document).ready(function($) {
    ...
    if ($(".RelatedList").length) {
        $('.RelatedList').owlCarousel({
            ...
            responsive: {
                0: {
                    items: $(".RelatedList").data("r0")
                },
                360: {
                    items: $(".RelatedList").data("r360")
                },
                560: {
                    items: $(".RelatedList").data("r560")
                },
                760: {
                    items: $(".RelatedList").data("r760")
                },
                960: {
                    items: $(".RelatedList").data("r960")
                },
                1360: {
                    items: $(".RelatedList").data("r1360")
                },
                1600: {
                    items: $(".RelatedList").data("r1600")
                }
            }
        });
    }
        ...
        a.event.special.mousewheel = {
            setup: function() {
                if (this.addEventListener)
                    for (var a = b.length; a;) this.addEventListener(b[--a], d, false);
                else this.onmousewheel = d
            },
            teardown: function() {
                if (this.removeEventListener)
                    for (var a = b.length; a;) this.removeEventListener(b[--a], d, false);
    ...
    if ($(".TrvideoFirst").length) {
        $('.TrvideoFirst').html(atob($('.TrvideoFirst').html()));
    }
